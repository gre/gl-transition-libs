...
const pickPositionsForSize = (w: number, h: number): [number, number][] => {
...
const expectedDraw1Picks: Color[] = [
  [4, 4, 0, 255],
  [251, 4, 0, 255],
  [4, 251, 0, 255],
  [251, 251, 0, 255],
...
const expectedDraw2Picks: Color[] = [
  [4, 4, 255, 255],
  [251, 4, 255, 255],
  [4, 251, 255, 255],
  [251, 251, 255, 255],
...
const debugColor = (c: Color, lbl: number) => {
  const [r, g, b, a] = c;
  return `${lbl}:rgba(${[r, g, b, a].join(",")})`;
};

const debugPicks = (picks: Color[], tests: boolean[]) =>
  picks
...
export default (gl: WebGLRenderingContext) => {
  ...
  function colorAt([x, y]: [number, number]): Color {
    const i = (x + y * w) * 4;
    const r = pixels[i + 0];
    const g = pixels[i + 1];
    const b = pixels[i + 2];
  ...
  return ({ glsl, paramsTypes, defaultParams }: CompilerInput): CompilerResult => {
    const data = {
      compileTime: 0,
      drawTime: 0,
    };
  ...
  void main () {
    gl_FragColor = transition(_uv);
      ...
      const afterCompilation = performance.now();
      ...
      for (const key in shader.types.uniforms) {
      ...
      gl.readPixels(0, 0, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, pixels); // this is just to force trigger a flush
      const beforeDraw1 = performance.now();
      ...
      const afterDraw1 = performance.now();
      const draw1PixelsPicks = pickPositions.map(colorAt);

      const beforeDraw2 = performance.now();
      shader.uniforms.progress = 1;
      gl.drawArrays(gl.TRIANGLES, 0, 3);
      gl.readPixels(0, 0, w, h, gl.RGBA, gl.UNSIGNED_BYTE, pixels);
      const afterDraw2 = performance.now();
