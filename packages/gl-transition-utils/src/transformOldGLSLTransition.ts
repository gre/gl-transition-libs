...
export interface TransformOldGLSLError {
  type: string;
  message: string;
}

export interface TransformOldGLSLResult {
  data: { glsl: string } | null;
  errors: TransformOldGLSLError[];
}
...
const isMainFunctionDefinitionAt = (tokens: Token[], i: number): boolean => {
...
const isFromOrToTexture2DCallAt = (tokens: Token[], i: number): boolean => {
...
export default (glsl: string): TransformOldGLSLResult => {
  const errors: TransformOldGLSLError[] = [];
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (token.type === "preprocessor" && token.data.indexOf("#ifdef") === 0) {
      ...
      const foundToRemove: Token[] = [];
      const foundToKeep: Token[] = [];
      let j;
      for (
        j = i + 1;
      ...
      if (foundToRemove.length > 0) {
        if (foundToKeep.length > 0) {
          errors.push({
            ...
            message:
              "Please manually removes uniforms: " +
