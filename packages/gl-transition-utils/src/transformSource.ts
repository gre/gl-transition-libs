...
export interface TransitionObject {
  ...
  name: string;
  glsl: string;
  defaultParams: { [key: string]: UniformDefaultValue };
...
const whitelistMeta = ["author", "license"] as const;
...
interface TypeInfo {
  primitiveType: "float" | "int" | "bool" | "sampler2D";
  exampleValue?: string;
  defaultValue: UniformDefaultValue;
  arity: number;
}

export const typeInfos: { [type: string]: TypeInfo } = {
  float: {
    primitiveType: "float",
    exampleValue: "0.7",
    defaultValue: 0,
...
function typeCheckTransitionFunction(node: GlslAstNode): boolean {
  node = node.parent;
  ...
  const functionargs = node.children.find((n) => n.type === "functionargs");
  if (!functionargs) {
    return false;
  }
  if (functionargs.children.length !== 1) {
    return false;
  }
  const decl = functionargs.children[0];
  if (decl.type !== "decl") {
    return false;
  }
  const args = decl.children.filter((n) => n.type !== "placeholder");
  if (args.length !== 2) {
    return false;
  }
...
export default function transformSource(
  ...
  const tokens: Token[] = tokenize(glsl);
  ...
  try {
    ...
    errors.push({
      type: "error",
      code: "GLT_GLSL_error",
      message: "GLSL code error: " + message,
      line,
    });
  }
    ...
    if (!ast.scope.transition) {
      ...
      if (!typeCheckTransitionFunction(ast.scope.transition)) {
        errors.push({
          ...
          message:
            "transition must be a function with following signature: 'vec4 transition(vec2 uv)'",
  ...
  function parseUniformCommentDefault(
    ...
    uniformId: string,
    uniformToken: Token
  ): UniformDefaultValue | undefined {
    comment = comment.trim();
    if (comment.indexOf("=") !== 0) {
      return;
    }
    const tokens: Token[] = tokenize(uniformId + " " + comment + ";");
    ...
    try {
      ...
      errors.push({
        ...
        message: `uniform '${uniformId}' default value '${comment}' does not parse as GLSL code: ${(e as Error).message}`,
    ...
    const step = (
      n: GlslAstNode | undefined,
      type: string,
      childIndex: number
    ): GlslAstNode | undefined =>
      n && n.type === type ? n.children[childIndex] : undefined;

    const valueNode = step(
      step(step(step(ast, "stmtlist", 0), "stmt", 0), "expr", 0),
      "assign",
      1
    );
    if (!valueNode) {
    ...
    function literalToJSValue(
      node: GlslAstNode
    ): UniformDefaultLiteralValue | undefined {
      ...
      switch (primitiveType) {
        case "float": {
          const f = parseFloat(literalNode.data);
          if (isNaN(f)) {
            errors.push({
              type: "error",
              code: "GLT_invalid_default_value",
    ...
    if (valueNode.type === "call") {
      const values: UniformDefaultLiteralValue[] = [];
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (token.type === "keyword" && token.data === "uniform") {
      ...
      let parsingType = true;
      const commentsPerIdent: { [ident: string]: string[] } = {};
      const commentsAll: string[] = [];
      ...
      while (++i < tokens.length) {
        ...
        if (token.type === "block-comment" || token.type === "line-comment") {
          if (ident) {
            commentsPerIdent[ident] = (commentsPerIdent[ident] || []).concat([
              extractCommentNode(token),
            ]);
          }
          continue;
        }
      ...
      idents.forEach((ident) => {
        ...
        if (typeof type !== "string" || !(type in typeInfos)) {
          ...
          let defaultParam: UniformDefaultValue | undefined;
          ...
          if (type === "sampler2D") {
            ...
            const comments = (commentsPerIdent[ident] || []).concat(
              commentsAll
            );
            for (
              let j = 0;
    ...
    if (token.type === "line-comment") {
      ...
      const m = com.match(/^(.*):(.*)$/);
      if (m && m.length === 3) {
        const key = m[1].trim().toLowerCase() as "author" | "license";
        const value = m[2].trim();
        if (!data[key] && (whitelistMeta as readonly string[]).indexOf(key) !== -1) {
          if (!value) {
            errors.push({
              type: "error",
